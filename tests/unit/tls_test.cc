...
SEASTAR_TEST_CASE(test_alpn_client_negotiate_h2_with_google,
    ...
    BOOST_CHECK(!selected_alpn.empty()); // Check that *some* protocol was negotiated
...
SEASTAR_TEST_CASE(test_alpn_client_server) {
    ...
    co_await when_all(
        [server = std::move(server)]() mutable -> future<> {
            ...
            auto server_side_alpn = co_await tls::get_selected_alpn_protocol(s.connection);
            BOOST_CHECK_EQUAL(server_side_alpn, "h2");
            co_return;
        },
        [addr]() mutable -> future<> {
            tls::credentials_builder b;
            ...
            auto client_side_alpn = co_await tls::get_selected_alpn_protocol(c);
            BOOST_CHECK_EQUAL(client_side_alpn, "h2");
            co_return;
        }
    );
    co_return;
