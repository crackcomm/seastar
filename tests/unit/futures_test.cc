...
SEASTAR_TEST_CASE(test_futurize_from_tuple_const_ref) {
    ...
    using type = unmovable_object;
    type n{5};

    // Ensure the support through `make_ready_future` as well.
    BOOST_REQUIRE_EQUAL(make_ready_future<type&>(n).get().v, 5);

    auto f = make_ready_future<>();
    f = std::move(f).then([&]() -> type& { return n; }).then([](type& x) { BOOST_REQUIRE_EQUAL(x.v, 5); });
    f = std::move(f).then([&]() -> const type& { return n; }).then([](const type& x) { BOOST_REQUIRE_EQUAL(x.v, 5); });
